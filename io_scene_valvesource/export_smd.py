...
'''.format(vca.bone_id, name,
    ...
    def writeDMX(self, datablock : bpy.types.ID, bake_results : list[BakeResult], name : str, dir_path : str):
        ...
        for bake in [bake for bake in bake_results if bake.object.type != 'ARMATURE']:
            ...
            jointCount = 0
            weight_link_limit = 4 if source2 else 3 # recent update from CS2 can increase weight link from 4 to 8 but it is not enabled by default without RenderMeshMarkup
            badJointCounts = 0
            have_weightmap = False
            cloth_groups = findDmxClothVertexGroups(ob) if source2 else None

            # replaced the DMX culling to per-mesh weight normalize
            if type(bake.envelope) is bpy.types.ArmatureModifier:
                ...
                for vert_weights in ob_weights:
                    ...
                    if weight_link_limit and count > weight_link_limit: badJointCounts += 1
                    jointCount = max(jointCount,count)
                if jointCount: have_weightmap = True
            elif bake.envelope: jointCount = 1

            if badJointCounts: self.warning(get_id("exporter_warn_weightlinks_excess",True).format(badJointCounts,bake.src.name,weight_link_limit))
            ...
            for (mat_name,indices) in bm_face_sets.items():
                ...
                if not material_elem:
                    ...
                    material_elem["mtlName"] = os.path.join(material_path, mat_name).replace('\\', '/')
        ...
        if is_anim: # animation
            ...
            def makeChannel(bone):
                ...
                for template in channel_template:
                    channel_name = boneExportName + template[0]
